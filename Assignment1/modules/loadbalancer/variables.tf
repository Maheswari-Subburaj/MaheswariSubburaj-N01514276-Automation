...
variable "loadbalancer-pubip" {
    default = "4276-loadbalancer-pubip"
}

variable "loadbalancer" {
    type = map(string) 
    default = {
        name    = "4276-loadbalancer"
        frontend_ip_configuration = "pubip-4276"
    }
}

variable "backend_address_pool" {
    default = "4276-backendaddresspool"
}

variable "loadbalancer_pool_association" {
    default = "4276-loadbalancer-pool-association"
}

variable "loadbalancer_rule" {
    type = map(string) 
    default = {
        name = "4276-loadbalancer-rule"
        protocol = "Tcp"
        frontend_port = 3389
        backend_port = 3389
